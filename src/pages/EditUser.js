...
const EditUser = () => {
 ...
 useEffect(() => {
  const fetchUserData = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`https://mern-backend-o430.onrender.com/api/users/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`https://mern-backend-o430.onrender.com/api/users/${id}`, { name, email, phone, status });
